...
class Evaluator(object):
    ...
    def evaluate(
        ...
        for model_name in self.eval_models:
            ...
            for _ in range(num_eval):
                ...
                print('*')
                for idx in range(0,len(compressors)):#实际上compressor set里面只有一个compressor
    ...
    def my_evaluate(
        ...
        if len(self.eval_models)==1 and len(compressors)==1:
            model_name=self.eval_models[0]
            accs = []
            print('Evaluate model ', model_name)
            for _ in range(num_eval):
                backbone = get_network(
            ...
            raise ValueError('Error: Please check the evaluate function you use')
