...
def my_organize_dst(dst_train, num_classes, print_info=True, dset_name='none', split='train', im_size=None,debug=False):
    print("organizing dst>>>>>>>>>>>>>>>>>>")
    images_all = []
    labels_all = []
    indices_class = [[] for c in range(num_classes)]
    ...
    if debug:
        ...
        load_num=len(dst_train)
    if data_loaded is None:
        # images_all = [torch.unsqueeze(dst_train[i][0], dim=0) for i in range(len(dst_train))]
        # labels_all = [dst_train[i][1] for i in range(len(dst_train))]
        images_all = [torch.unsqueeze(dst_train[i]['image'], dim=0) for i in range(load_num)]
        labels_all = [dst_train[i]['label'] for i in range(load_num)]
        images_all = torch.cat(images_all, dim=0)
    else:
        images_all = data_loaded['images_all']
        labels_all = data_loaded['labels_all']
    ...
    if print_info:
        for c in range(num_classes):
            print('class c = %d: %d real images'%(c, len(indices_class[c])))
    # 注意每个类别的张数不一样 应该是整个样本集是每个类别700张 训练集每个类别400张左右
        for ch in range(images_all.shape[1]):
            print('real images channel %d, mean = %.4f, std = %.4f'%(
                ch, torch.mean(images_all[:, ch]), torch.std(images_all[:, ch])))
    ...
    if dataset == 'MNIST':
        ...
        if zca:
            ...
            def transform_fn(batch):
                ...
                return batch
            dst_train=load_dataset(data_path, split='train')
            dst_train.set_transform(transform_fn)
            dst_test=load_dataset(data_path, split='test')
            dst_test.set_transform(transform_fn)
            # warning
